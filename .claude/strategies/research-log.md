...
GLD/IAU/SLV have the same signal because they track the same underlying. Running all three simultaneously isn't three independent bets — it's one bet with 3x size. Correlation-aware position sizing (reduce per-trade risk when 3+ correlated bots in simultaneously) is required before real money.
...
8. **Correlation-aware sizing algorithm** — design and test the position sizing logic that reduces per-trade risk when 3+ correlated bots are simultaneously in. Required before real money regardless of other progress.
