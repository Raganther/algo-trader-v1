...
>
> **Bot lineup framing (added Apr 28 evening):** the 7 deployed bots represent ~3 independent economic bets — gold cluster (GLD/IAU/SLV/GDX), energy cluster (OIH/XOP), and biotech (XBI). Capital cap binds at 4 simultaneous full positions (25% notional × 4 = 100% equity). **Adding more bots is NOT the next move** — it's "let the existing 7 produce live data + build the supporting tooling." See "Live Observation Framework" section below for what to measure. Adding bots is gated on (a) correlation-aware sizing landing first, (b) the new bot adding an independent cluster, not a correlated one. Only IWM currently meets the second criterion.
...
| Metals regime detector v1 | idea | 200-SMA + 14-period ADX on daily metals bars → classify as TRENDING_UP / RANGING / TRENDING_DOWN / HIGH_VOL. If TRENDING_DOWN, downsize metals bots automatically (e.g. 1% risk instead of 2%, or skip entries entirely). Existing `regime_classifier.py` already has the logic — wire it into runner.py per-bot. Lower priority than correlation-aware sizing but cheap to add once the latter ships. |
| Real-money pilot ($1–5k) when ready | idea — gating | Once: (1) 6+ months live forward test on 7 bots within 30% of backtest, (2) correlation-aware sizing built and tested, (3) live slippage measured and incorporated into backtest model. Pilot at small real capital to measure execution friction at scale-of-real. **Not a near-term action.** |
...
| WF validation for held-out passers + SPY/QQQ/IWM/DIA | **deprioritised** — not a deployment path | Apr 28 evening reframing: the 16 single-run passers (12 held-out + 4 indices) are NOT viable deployment candidates without (a) WF validation, (b) Sharpe verification per-asset, (c) random-entry test on each, (d) inversion test to check regime-dependence, (e) correlation analysis vs existing 3 clusters. **Of the 16, only IWM has Sharpe ≥ 2.0 verified AND direction-agnostic profile (Test 3) AND low correlation to the existing lineup.** IWM is the *only* sensible single-bot expansion candidate, and it's still gated on correlation-aware sizing. The other 15 are interesting data points, not next bots. |
