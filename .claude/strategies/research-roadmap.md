...
| Item | Status | Notes |
|------|--------|-------|
| Held-out test on 12 novel assets (XLF/XLV/XLI/XLK/KRE/UUP/GBTC/EWZ/ITA/VXX/ARKK/TQQQ) | **complete — interpretation pending** | All 12 produced positive returns single-run on validated recipe + extended window. Range +10% (UUP) to +200% (VXX). Three predicted failures (VXX/TQQQ/ARKK) all passed strongly. Either the strategy is universally robust on liquid ETFs (much stronger claim than we'd previously made) or the test isn't distinguishing real edge from artefact. See `research-log.md` → "Held-Out Generalisation Test" for full data + interpretation. |
| **Boundary verification: re-run SPY/QQQ/IWM/DIA/TLT with validated recipe** | **urgent** | Cross-cutting learning #10 ("fails on broad equity indices") was based on old params + broken engine. Never re-tested. If these pass with validated recipe, the boundary thesis is illusory and the strategy is general. If they still fail, the held-out test is genuine validation. **5 backtests, 5 minutes — single most informative test in the queue.** |
...
| WF validation for held-out passers (if SPY/QQQ retest confirms boundary illusory) | conditional | If the boundary thesis breaks, then VXX/TQQQ/ARKK/XLK/KRE etc. become real candidates rather than data points. WF + Sharpe + correlation analysis would be needed before any deployment decision. Defer until boundary verified. |
