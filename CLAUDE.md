...
- `.claude/strategies/trail-anchor-hwm.md` — read when considering trail-stop changes, deciding whether to flip live bots to HWM, or recalibrating validated-edges Sharpe figures. **May 7 result: HWM trail anchor delivers +0.78 Sharpe (4.95 → 5.73) and −0.36pp DD (3.41% → 3.05%) on long-window 7-bot backtest.** All 7 symbols improve. Default kept at 'close' for backward compatibility; HWM opt-in via `"trail_anchor": "hwm"` parameter. Sharpe lift ≈ the 0.7 backtest-vs-live delay artifact, suggesting HWM structurally bypasses the 1-bar phase-shift problem. Live deployment pending strategic decision.
