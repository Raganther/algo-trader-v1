...
- **Portfolio-level total-notional cap shipped** (`correlation_sizing.portfolio_cap_max_size`, helper at `backend/engine/correlation_sizing.py`, CLI flag `--portfolio-cap-frac N`, sizing blocks `min(risk, 25%-per-pos, cluster_max, portfolio_max)`). Module toggle `PORTFOLIO_CAP_ENABLED` default OFF currently; **recommended flip to default ON at FRAC=1.0**. Run A confirms it's a tiny positive (+0.09 Sharpe, −0.17pp DD) on the 7-bot lineup — the cap binds on gold N=4 stacking (4.2% of bars). It's also the structural prerequisite for any universe-expansion experiment to produce deployable return numbers.
