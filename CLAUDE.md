...
**Shared-timeline portfolio runner V1 (shipped Apr 29 2026 evening):** `backend/engine/portfolio_runner.py` + `python3 -m backend.runner portfolio ...`. One PaperTrader, N strategy instances on a unified time grid. **First V1 finding: gold cluster has ≥2 members open on 46.7% of bars (≥3 on 20.1%) over 2020-07 → 2026-04 — the correlation discount fires materially.** Snapshot: `.claude/strategies/portfolio-runner-baseline.md`. **V1 caveat:** total return / Sharpe figures are model upper-bounds (no per-bot allocation cap → shared-capital compounds aggressively); apples-to-apples comparisons across configurations are still valid. V2 (per-bot cap) is the next planned change before quoting headline portfolio Sharpe.
