...
> **May 7 2026 caveat (refined May 8) — backtest delay artifact + HWM lift.** Sharpe figures below are **close-anchored backtests with ~0.4–0.7 Sharpe optimism baked in** (live's 1-bar polling delay not modelled; May 8 falsification audit reproduced 0.42 of the predicted 0.7). HWM trail anchor (`trail_anchor: 'hwm'`, opt-in May 7) lifts long-window 7-bot Sharpe 4.95 → 5.73 (+0.78). May 8 audit confirms HWM is more delay-resistant (~2.8×) but not delay-immune: live HWM gap likely ~0.2–0.3 Sharpe, not zero. Per-asset Sharpes have not yet been re-run with HWM. See `.claude/calibration/audit-hwm-delay-mechanism.md`, `.claude/calibration/live-vs-backtest-iau-diagnostic.md`, `.claude/strategies/trail-anchor-hwm.md`.
>
