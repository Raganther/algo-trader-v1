...
def run_portfolio(args):
    ...
    md.append("")
    md.append("> **Interpretation note (V2 — fixed-equity reference, Apr 30 2026).**")
    md.append("> Each strategy sizes risk + notional cap off `initial_capital` ($94k), not")
    md.append("> the compounding pool. This mirrors live mechanics: 7 bots on a single")
    md.append("> Alpaca account each see the same equity number when sizing. The 25%")
    md.append("> per-position notional cap × 4 simultaneous positions = 100% notional ceiling")
    md.append("> matches the live deployment binding constraint exactly. No compounding.")
    md.append(">")
    md.append("> Single-symbol runs use `equity_mode='live'` (current equity, V1 behaviour)")
    md.append("> and are byte-identical to single-symbol backtests. The portfolio runner")
    md.append("> overrides this to `'fixed'` for multi-symbol runs.")
    md.append(">")
    md.append("> **About Sharpe.** Sharpe is sizing-invariant — scaling every position by a")
    md.append("> constant scales mean return and stdev equally, so the ratio is unchanged.")
    md.append("> V1's Sharpe was *not* an upper-bound artefact (only its return / DD were).")
    md.append("> The portfolio Sharpe shown here is real and reflects diversification across")
    md.append("> imperfectly-correlated bots (≈ √N × asset Sharpe at low cross-correlation).")
    md.append("")
